...
function shuffleCardImageClasses() {
   ...
   */
   let shuffledArray = _.shuffle(cardArray);
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++){
    ...
    let cardCreation = appendNewCard(parentElement);

    // Step 2(b): Add an image class to the new card element, using shuffledImageClasses[i].
    cardCreation.classList.add(shuffledImageClasses[i]);
...
function onCardFlipped(newlyFlippedCard) {
  ...
  if ((document.getElementById("match-count").innerHTML >=1) || (document.getElementById("match-count").innerHTML <=5)) {
    matchAudio.play();

    // Level Up: Adding glow effect to matching cards
